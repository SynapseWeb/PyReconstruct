...
- [ ] Dim 6 transformations (retro-reconstruct before truncating and importing)

- [ ] Modes (change to strings, e.g., "none", "filled", etc.)

- [ ] Selected objects from the inside
   - [ ] Consider adding selected object list to field opposite trace palette

- [ ] Object list
   - [ ] Right click on objec and access trace attribute menu

- [ ] Section list
   - [ ] Consider section attribute /type/ = ("Tissue", "Cal-grid", ...)

- [ ] Save notification - increase time out so user has a chance to read message
...
- [ ] Zoom when control pressed to mouse position (don't lift ctrl)
