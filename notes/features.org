...
- [ ] Object list
   - [ ] Round to 5 decimels (not sig figs)
   - [ ] Edit object attributes by right clicking
   - [ ] Add objects to object groups in attributes menu
   - [ ] Export (under "list menu") as CSV
   - [ ] Consider removing refesh
   - [ ] Edit object list variables from the object list
   - [ ] Add group variable column
   - [ ] Add tags to all traces associated with an object (don't overwrite any existing tags)
   - [ ] Add trial 3D render to object list
   - [ ] Standard regexp tick boxes to filter
     
- [ ] Move traces in field
...
- [ ] Trace list
   ...
   - [ ] Tags
      ...
      - [ ] Consider always tagging new traces with =os.login()= as default
      - [ ] Option to change username (default to =os.login()=)
     
- [ ] Field attribute menu
   - [ ] Access by right clicking trace
   - [ ] Consider allowing user to change trace and object attributes from that window
     
- [ ] Tool palette
   - [ ] Add keyboard shortcuts for tool (s for select, z for pan/zoom, etc.)
   - [ ] Add keyboard shorts for trace buttons (1 for first, 2 for second, etc.)
   - [ ] Ctrl-numbers to access trace button attributes from the field
