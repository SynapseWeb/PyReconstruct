...
class ObjectVolume():
    ...
    def generateVolumes(self, alpha=1):
        ...
        for obj_name, obj_dict in self.obj_data.items():
            ...
            # generate the mesh from the volume
            ...
            # pv_faces = np.hstack(np.column_stack((
            #     np.array([3]*len(faces)),
            #     faces
            # )))

            # surf = PolyData(verts, pv_faces)

            # print("attempting to smooth")

            # surf.smooth(n_iter=1000, relaxation_factor=0.5)
