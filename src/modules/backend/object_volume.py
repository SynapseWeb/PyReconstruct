...
class ObjectVolume():
    ...
    def generateVolume(self, mag, alpha : int = 255) -> tuple[gl.GLVolumeItem, tuple, tuple]:
        ...
        if not self.uniform_section_thickness:
            ...
            for snum in self.sections:
                st = self.sections[snum]["thickness"]
                z_sections[snum * st] = self.sections[snum]
            self.sections = z_sections

        # get the minimum and maximum x, y, z
        xmin, ymin, zmin, xmax, ymax, zmax = tuple(self.extremes)

        # calculate the desired maginification based on the final volume
        if self.uniform_section_thickness:
            vshape = (int(mag*(xmax-xmin)+1), int(mag*(ymax-ymin)+1), zmax-zmin+1, 4)
        else:
            vshape = (int(mag*(xmax-xmin)+1), int(mag*(ymax-ymin)+1), int(mag*(zmax-zmin)+1), 4)
        ...
        for z in self.sections:
            ...
            for contour in self.sections[z]["contours"]:
                # keep track of the negative traces
                negative_traces = []
                for trace in self.sections[z]["contours"][contour]:
        ...
        if self.uniform_section_thickness:
            ...
            ys = 1 / mag
            zs = 1 / mag
        vol_item.scale(xs, ys, zs)

        # translate the item to its actual position
        vol_item.translate(xmin, ymin, )
