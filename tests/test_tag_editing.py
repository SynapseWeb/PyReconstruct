...
class _FakeTableManager:
    def activeTable(self, table_class):
        # No object list is acting, so object_function falls back to the
...
class _FieldStub:
    ...
    def __init__(self, series, obj_names):
        ...
        self.table_manager = _FakeTableManager()
...
def test_trace_list_tags_column_is_ordered():
    """The third place a set was rendered straight into the interface.

    ``getItems`` is called with the row's data only, so it runs unbound here
    rather than behind a table widget.
    """
    ...
    items = TraceTableWidget.getItems(
        None, ("axon", _TraceData()), "Tags"
    )

    assert [i.text() for i in items] == ["alpha, mu, zeta"]


# --- a selection whose traces disagree on their tags -----------------------
#
# The dialog shows one blank Tags field for such a selection, because there is
# no single set of tags to put in it. Every other field has a value that means
# "no single value, leave it alone" -- "*" for the name, None for the color --
# but the tag field's blank state used to be reported as an empty set, which
# ``editTraceAttributes`` applies: changing the color of two differently tagged
# traces stripped the tags off both.


def _mixed_selection():
    return [_trace({"check"}), _trace({"reviewed"})]


def test_dialog_reports_a_disagreeing_selection(qapp):
    dialog = TraceDialog(None, traces=_mixed_selection())

    assert dialog.tags_differ
    assert [w.text() for w in dialog.tags_input.inputs] == [""]
    assert dialog.tags_input.inputs[0].placeholderText(), (
        "a blank field that means 'these differ' has to say so"
    )

    dialog.deleteLater()


def test_agreeing_selection_is_not_flagged(qapp):
    dialog = TraceDialog(None, traces=[_trace({"check"}), _trace({"check"})])

    assert not dialog.tags_differ
    assert [w.text() for w in dialog.tags_input.inputs] == ["check"]

    dialog.deleteLater()


def _accept(dialog, monkeypatch):
    """Run the exec() body without the modal loop, as the user pressing OK."""
...
def test_untouched_tags_field_reports_no_change(qapp, monkeypatch):
    """The bug: OK without touching the Tags field must not clear any tags."""
    dialog = TraceDialog(None, traces=_mixed_selection())

    trace, confirmed = _accept(dialog, monkeypatch)

    assert confirmed
    assert trace.tags is None, (
        "an untouched field on a disagreeing selection means 'leave the tags "
        "alone', which is None -- an empty set is applied and clears them"
    )

    dialog.deleteLater()


def test_edited_tags_field_replaces(qapp, monkeypatch):
    """Typing into the field makes it the intended final list."""
    dialog = TraceDialog(None, traces=_mixed_selection())
    dialog.tags_input.inputs[0].setText("axon")
    dialog.tags_input.edited = True  # what typing does

    trace, _ = _accept(dialog, monkeypatch)

    assert trace.tags == {"axon"}

    dialog.deleteLater()


def test_emptied_tags_field_still_clears_a_disagreeing_selection(
    qapp, monkeypatch
):
    """Pressing "-" on the blank field is an edit, so it can still clear.

    Otherwise there would be no way to take the tags off a mixed selection.
    """
    dialog = TraceDialog(None, traces=_mixed_selection())
    dialog.tags_input.remove()
...
def test_single_trace_can_still_be_cleared(qapp, monkeypatch):
    """The field showed what it was clearing, so no edit flag is needed."""
    dialog = TraceDialog(None, traces=[_trace({"axon"})])
...
def test_multi_input_tracks_user_edits(qapp):
    from PySide6.QtWidgets import QWidget

    host = QWidget()

    field = MultiInput(host, ["axon"])
    assert not field.edited, "prefilling the field is not an edit"

    field.inputs[0].setText("spine")
    assert not field.edited, (
        "setText is how the field is prefilled; only textEdited, which Qt emits "
        "for user input alone, can mark it"
    )
...
def test_combo_field_tracks_user_edits(qapp):
    """The group and host fields are comboboxes over the same widget."""
    from PySide6.QtWidgets import QWidget

    host = QWidget()
    field = MultiInput(host, ["axon"], combo=True, combo_items=["axon", "spine"])

    assert not field.edited

    field.inputs[0].setCurrentText("spine")
    assert field.edited

    host.deleteLater()


# --- tags are not shared between the traces they are applied to ------------


def _section_with(traces):
    """A real Section holding the given traces, nothing stubbed but the series."""
    ...
    section.mag = 1.0
    for t in traces:
        section.contours.setdefault(t.name, Contour(t.name)).append(t)
    return section


def test_edited_traces_do_not_share_one_tag_set():
    """Each trace needs its own set.

    ``editTraceAttributes`` assigned the caller's set to every trace it touched,
    so a later in-place add -- the additive branch below, the knife's tag merge,
    the import conflict flagger -- reached traces the caller never named. Pasting
    attributes shares the set with the clipboard trace on top of that.
    """
    section = _section_with([_trace({"axon"}), _trace({"axon"})])
    traces = section.contours["axon"].getTraces()

    section.editTraceAttributes(
        traces.copy(), None, None, {"axon"}, None, log_event=False
    )
    edited = section.contours["axon"].getTraces()

    assert edited[0].tags == edited[1].tags == {"axon"}
    assert edited[0].tags is not edited[1].tags

    edited[0].tags.add("only_the_first")
    assert edited[1].tags == {"axon"}


def test_the_callers_set_is_not_captured():
    """The set passed in stays the caller's own."""
    section = _section_with([_trace({"axon"})])
    passed = {"axon"}

    section.editTraceAttributes(
        section.contours["axon"].getTraces().copy(),
        None, None, passed, None, log_event=False,
    )
    edited = section.contours["axon"].getTraces()[0]

    assert edited.tags is not passed
    edited.tags.add("added_later")
    assert passed == {"axon"}
